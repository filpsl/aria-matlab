 ...
 * value.)
 * 3. cycle counter
 * 4. Include access to ArRobot callbacks including robot cycle tasks and
...
#define AR_ASSERT_RETURN(ptr)                                                  \
  {                                                                            \
    if (!ptr) {                                                                \
      ArLog::log(ArLog::Terse, "ariac: failed assertion: (%s) at %s:%d", #ptr, \
                 __FILE__, __LINE__);                                          \
      return;                                                                  \
    }                                                                          \
  }
...
// Switch the following to print debugging information at the start of every
// function call
/*
#ifdef _MSC_VER
...
AREXPORT void arloginfo(const char *m) {
  ...
  ArLog::log(ArLog::Terse,
             "%s: init=%d connected=%d argParser=0x%x robot=0x%x "
             "robotConnector=0x%x argc=%d argv=0x%x (%s)",
             m, init, connected, argParser, robot, robotConnector, argc, argv,
             args.c_str());
  if (robot)
...
AREXPORT int arrobot_connect() {
  AR_DEBUG_LOGINFO();
  if (!init) {
    ArLog::log(ArLog::Terse, "arrobot_connect: Error: aria_init() must be "
                             "called before arrobot_connect().");
    return 0;
  }
  if (connected) {
  ...
  if (!robotConnector->connectRobot()) {
    ArLog::log(
        ArLog::Normal,
        "arrobot_connect: Could not connect to the robot. Check connections or "
        "set connection options as program arguments in init.");
  ...
  ArLog::log(ArLog::Normal,
             "arrobot_connect: Connected to robot. ArRobot processing cycle is "
...
AREXPORT void arrobot_disconnect() {
  ...
  connected = false;
  if (!robot) {
    ArLog::log(ArLog::Terse, "arrobot_disconnect: internal error: NULL robot!");
    return;
  }
  ...
  if (!robotConnector) {
    ArLog::log(ArLog::Terse,
               "arrobot_disconnect: internal error: NULL robotConnector!");
    return;
  }
...
AREXPORT void arrobot_get_bumpers(char *front, char *rear) {
  ...
  if (front)
    *front = ((robot->getStallValue() & 0xff00) >> 8) >>
             1; // second byte is front bumpers; BIT0 is the left stall flag,
                // shift over it
  if (rear)
    *rear =
        (robot->getStallValue() & 0x00ff) >>
        1; // LSB is front bumpers; BIT0 is the right stall flag, shift over it
}
...
AREXPORT void arrobot_resetpos() {
  ...
  ArLog::log(ArLog::Normal,
             "arrobot_resetpos: Reset robot odometery to origin and "
             "transformed ArRobot pose, it is now %0.2f, %0.2f, %0.2f.\n",
